...
RAW_RESULTS_FILE = "smollm3_3b_fp16_2048_30runs_raw.csv"
GPU_SAMPLES_FILE = "smollm3_3b_fp16_2048_30runs_gpu_samples.csv"
PAIRED_FILE = "smollm3_3b_fp16_2048_30runs_paired.csv"
SUMMARY_FILE = "smollm3_3b_fp16_2048_30runs_summary.csv"
ENVIRONMENT_FILE = "smollm3_3b_fp16_2048_30runs_environment.txt"
...
def get_torch_memory_stats():
    ...
    return {
        "torch_allocated_GiB":
            bytes_to_gib(torch.cuda.memory_allocated()),

        "torch_reserved_GiB":
            bytes_to_gib(torch.cuda.memory_reserved()),

        "torch_max_allocated_GiB":
            bytes_to_gib(torch.cuda.max_memory_allocated()),

        "torch_max_reserved_GiB":
            bytes_to_gib(torch.cuda.max_memory_reserved())
    }
...
class GPUMonitor:
    ...
    def sample_once(self, stage):
        ...
        if "error" not in gpu:
            ...
            sample.update(
                {
                    "GPU_Temperature_C":
                        safe_float(gpu.get("temperature.gpu")),

                    "GPU_Utilization_percent":
                        safe_float(gpu.get("utilization.gpu")),

                    "Memory_Utilization_percent":
                        safe_float(gpu.get("utilization.memory")),

                    "NVIDIA_Memory_Used_MiB":
                        safe_float(gpu.get("memory.used")),

                    "NVIDIA_Memory_Total_MiB":
                        safe_float(gpu.get("memory.total")),

                    "SM_Clock_MHz":
                        safe_float(gpu.get("clocks.current.sm")),

                    "Memory_Clock_MHz":
                        safe_float(gpu.get("clocks.current.memory")),

                    "Power_Draw_W":
                        safe_float(gpu.get("power.draw")),
            ...
            sample["NVIDIA_SMI_Error"] = gpu["error"]
    ...
    def _monitor_loop(self):
        ...
        while not self.stop_event.is_set():
            ...
            self.sample_once(stage="during")
            self.stop_event.wait(self.interval)


    def start(self):

        self.sample_once(stage="before")
...
print()
print("=" * 80)
print("SMOLLM3-3B FP16 KV-CACHE 30+30 BENCHMARK")
print("=" * 80)
...
print(f"Generated tokens/run: {MAX_NEW_TOKENS}")
print(f"Pairs: {NUM_PAIRS}")
print(f"Total measured runs: {NUM_PAIRS * 2}")


# ============================================================
# GPU BEFORE MODEL LOAD
# ============================================================

initial_gpu_status = get_nvidia_smi_stats()

print()
print("=" * 80)
print("NVIDIA-SMI BEFORE MODEL LOAD")
print("=" * 80)

for key, value in initial_gpu_status.items():
    print(f"{key}: {value}")
...
if hasattr(model, "get_memory_footprint"):
    ...
    hf_memory_bytes = model.get_memory_footprint()
    hf_memory_gib = bytes_to_gib(hf_memory_bytes)
...
gpu_after_model = get_nvidia_smi_stats()

print()
print("NVIDIA-SMI after model load:")

for key, value in gpu_after_model.items():
    print(f"{key}: {value}")
...
def create_exact_token_prompt():
    ...
    if len(base_ids) == 0:
        raise RuntimeError("Base text produced no tokens.")
...
print()
print("=" * 80)
print("RANDOMIZED PAIRED SCHEDULE")
print("=" * 80)
...
print()
print("=" * 80)
print("STARTING 30+30 BENCHMARK")
print("=" * 80)
...
for run_number, experiment in enumerate(
    ...
    pair_number = experiment["Pair"]
    use_cache = experiment["KV_Cache"]
    ...
    result, gpu_samples = benchmark_once(
        use_cache=use_cache,
        run_number=run_number,
        pair_number=pair_number
    )
    ...
    if "Max_Physical_VRAM" in result:
    ...
    # Save partial data after every run
...
gpu_samples_df = pd.DataFrame(all_gpu_samples)

off_df = results_df[
    results_df["KV_Cache"] == False
].copy()

on_df = results_df[
    results_df["KV_Cache"] == True
].copy()
...
def summarize_condition(
    ...
    tps = df["Tokens_per_sec"].to_numpy()
...
for pair_number in range(
    ...
    if (
        ...
        raise RuntimeError(
            f"Pair {pair_number} incomplete."
        )

    off_time = off_pair.iloc[0]["Time_s"]
    on_time = on_pair.iloc[0]["Time_s"]
...
mean_paired_speedup = paired_df[
    "Speedup"
].mean()

median_paired_speedup = paired_df[
    "Speedup"
].median()

sd_paired_speedup = paired_df[
    "Speedup"
].std(ddof=1)

mean_time_saved = paired_df[
    "Time_Saved_s"
].mean()

median_time_saved = paired_df[
    "Time_Saved_s"
].median()

sd_time_saved = paired_df[
    "Time_Saved_s"
].std(ddof=1)
...
off_outliers = iqr_outliers(
    off_times
)

on_outliers = iqr_outliers(
    on_times
)
...
off_sorted = off_df.sort_values(
    "Run"
)

on_sorted = on_df.sort_values(
    "Run"
)

off_spearman = stats.spearmanr(
    off_sorted["Run"],
    off_sorted["Time_s"]
)

on_spearman = stats.spearmanr(
    on_sorted["Run"],
    on_sorted["Time_s"]
)
...
if "Max_Physical_VRAM" in results_df.columns:

    maximum_vram = results_df[
        "Max_Physical_VRAM"
    ].max()
...
if not np.isnan(vram_percentage):

    print()
    print("=" * 80)
    print("VRAM SAFETY CHECK")
    print("=" * 80)
...
print()
print("=" * 80)
print("SMOLLM3 30+30 EXPERIMENT COMPLETE")
print("=" * 80)
